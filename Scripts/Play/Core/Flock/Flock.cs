...
public class Flock : MonoBehaviour
{
    ...
    private float AnimCheckTime = 0f;
    ...
    public void SetAnimation(FlockUnit.eAnimType inType)
    {
        //이미 애니메이션 상태면 다 못들어오게한다.
        if (IsAnimation)
            return;

        for (int i = 0; i < allUnits.Length; i++)
            allUnits[i].AnimChangeState(inType);

        //애니메이션 임의재생시간 이시간동안 물고기는 안움직인다.
        AnimCheckTime = 1f;
    }
